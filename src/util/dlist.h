...
public:
   ...
   void push_back(const T& x) {
      ...
      if (empty()){
         ...
         _isSorted = (_isSorted && x>=_head->_prev->_data)?true:false;
         DListNode<T>* t = new DListNode<T>(x,_head->_prev,_head);
         _head -> _prev -> _next = t;
         _head -> _prev  = t;
      }
      return;
   ...
   void sort() { 
      ...
      if (_isSorted || size()<=1) return;
         ...
         DListNode<T> *temp = _head->_next->_next, *temp_forward = temp;
         while(temp -> _next != _head->_next){
            temp_forward = temp;
            temp = temp -> _next;
            // cout << "temp: " << temp -> _data << endl;
            // cout << "tf: " << temp_forward -> _data << endl;
            // cout << "head: " << _head -> _data << endl;
            while (temp_forward -> _data < temp_forward -> _prev -> _data && temp_forward != _head->_next){
